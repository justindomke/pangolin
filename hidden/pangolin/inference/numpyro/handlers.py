...
def get_numpyro_rv(op: ir.Op, name: str, obs, *numpyro_pars):
    ...
    if op.random and not is_continuous(op) and not is_observed:
        raise ValueError(
            "NumPyro inference cannot handle unobserved discrete latent variables."
        )

    if op.random:
        # Why doesn't this check always work?
        # Answer: Because NumPyro does weird stuff when integrating out discrete RVs
        if obs is not None:
            assert (
                obs.shape == numpyro_val.shape or obs.shape == numpyro_val.shape()
            ), f"{obs.shape} vs {numpyro_val.shape}"
...
def handle_composite(op: ir.Composite, *numpyro_parents, is_observed):
    ...
    for n, (my_cond_dist, my_par_nums) in enumerate(
        zip(op.ops, op.par_nums, strict=True)
    ):
