...
def handle_vmap_random(op: ir.VMap, *numpyro_parents, is_observed):
    ...
    _support = jax.vmap(base_support, op.in_axes, axis_size=op.axis_size)(
        *numpyro_parents
    )

    eg_args = get_eg_args(op, *numpyro_parents)
    _has_enumerate_support = get_numpyro_val(
        op.base_op, *eg_args, is_observed=is_observed
    ...
    class NewDist(numpyro_dist.Distribution):
        ...
        def sample(self, key, sample_shape=()):
            ...
            if sample_shape == (1,):
                return jnp.array(
                    [jax.vmap(base_sample, in_axes, axis_size=axis_size)(*args)]
                )
            elif sample_shape == ():
                return jax.vmap(base_sample, in_axes, axis_size=axis_size)(*args)
            else:
                assert (
                    False
                ), "Can't handle this sample shape (Pangolin bug, please report!)"
...
def get_plate_size(op, *numpyro_pars):
    ...
    remaining_shapes, axis_size = ir.get_sliced_shapes(
...
def vmap_rv_plate(op, name, obs, *numpyro_pars):
    ...
    assert plate_vmap_compatible(
        op
    ), "op {op} not compatible with vmap_rv_plate (Pangolin bug)"
    ...
    out_shape = out.shape  # type: ignore
    expected_shape = op.get_shape(*[p.shape for p in numpyro_pars])
    assert (
        out_shape == expected_shape
    ), f"Shape match failed (Pangolin bug) {out_shape} vs {expected_shape} {plate_sizes=} {my_numpyro_pars=}"
    return out
