...
class Scanner(RV):
    ...
    def __init__(self, init, *parents, f):
        ...
        fake_downstream = middle_nodes(roots=fake_parents + [fake_old_carry], leafs=[fake_carry, fake_output]) + [
            fake_carry,
            fake_output,
        ]
        ...
        for fake_node in fake_downstream:
            ...
            real_nodes[fake_node] = ItemScanner(
                *my_parents, fake_parent=fake_node, vec_pars=my_vec_pars, N=myN, tracer_avoid=tracer
            )
    ...
    def getcode(self, loop_indices=None, par_loop_indices=None):
        ...
        code = (
            self.carry.ref(local_indices=["1"], loop_indices=loop_indices)
            + " <- "
            + init.ref(loop_indices=init_loop_indices)
            + ";\n"
        )
