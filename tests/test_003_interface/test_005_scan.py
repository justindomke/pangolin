...
from pangolin.interface.scanning import (
    scan,
    scan_flat,
    repeat,
)
...
def test_normal_external_scale():
    ...
    assert y.op.base_op == base_op
    assert y.op == ir.Scan(base_op, 10, [None])
...
def test_normal_with_param():
    ...
    y = scan_flat(fun, 10, (0,))(u, scales)
    base_op = ir.Composite(2, (ir.Normal(),), [[0, 1]])
    assert y.op.base_op == base_op
    assert y.op == ir.Scan(base_op, 10, [0])
...
def test_pass_constants():
    ...
    y = scan_flat(fun, 10, (0,))(2.2, np.arange(1, 11))
    base_op = ir.Composite(2, (ir.Normal(),), [[0, 1]])
    assert y.op.base_op == base_op
    assert y.op == ir.Scan(base_op, 10, [0])
...
def test_normal_with_unmapped_param_non_flat():
    ...
    for u in [u_np, u_rv]:
        for scales in [scales_rv, scales_np]:
            for in_axes in [None]:
                y = scan(fun, length, in_axes)(u, scales)
...
def test_interesting_scan():
    ...
    y = [[3, 4], 5]
    stuff = {"hi": 6, "yo": 7}
    y = scan(fun, length=10, in_axes=None)(init, x, y, stuff)
    ...
    assert y.op.base_op == base_op
    op = ir.Scan(base_op, 10, [None] * 7, 0)
    assert y.op == op


def test_interesting_closure_scan():
    ...
    assert y.op.base_op == base_op
    op = ir.Scan(base_op, 10, [None] * 6, 0)  # one less input
    assert y.op == op
...
#     cumsum = scan(lambda last, input: last + input)
#     vecwalk = scan(lambda last: cumsum(0.0, last), 5)
...
#     cumsum = scan(lambda last, input: normal(last + input, 1e-7))
#     vecwalk = scan(lambda last: cumsum(0.0,last), 5)
