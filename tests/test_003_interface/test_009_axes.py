...
def test_assign_range():
    ...
    assert x.op == ir.VMap(ir.Identity(), [0], 3)
    assert x.parent_ops == (ir.Constant(range(3)),)
...
def test_assign_exp_range():
    ...
    assert tmp.parent_ops == (ir.Constant(range(3)),)
...
def test_assign_normal_range():
    ...
    assert tmp.parent_ops == (ir.Constant(range(3)), ir.Constant(5.5))
