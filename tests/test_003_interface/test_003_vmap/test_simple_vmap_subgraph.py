...
def test_normal_all_none():
    ...
    c_dummy = RV(ir.Normal(), a_dummy, b_dummy)
    [c] = vmap_subgraph(
        [a_dummy, b_dummy], [c_dummy], [c_dummy], [a, b], [None, None], 3
    )
...
def test_normal_add():
    ...
    [d] = vmap_subgraph(
        [a_dummy, b_dummy], [c_dummy, d_dummy], [d_dummy], [a, b], [0, 0], 3
    )
    c = d.parents[0]
    assert c.parents == (a, b)
...
def test_normal_add_nomap():
    ...
    d_dummy = RV(ir.Normal(), c_dummy, a_dummy)
    [d] = vmap_subgraph(
        [a_dummy, b_dummy], [c_dummy, d_dummy], [d_dummy], [a, b], [None, None], 3
    )
    c = d.parents[0]
    assert c.parents == (a, b)
    assert d.parents == (c, a)
