...
def test_repeated_exp_with_dummy():
    length = 5
    ...
    sample_flat_until_match([y], [dummy], [0.1], testfun)
    sample_until_match(y, dummy, 0.1, testfun)
...
def test_autoregressive_const_rv_mapped():
    x = constant(0.5)
    length = 12
    noises = constant(np.random.rand(length))
