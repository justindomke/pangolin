...
class SimplePosteriorTests(HasInferenceProps):
    ...
    def test_simple(self):
        ...
        [x_samps] = self.sample_flat([x], [], [], niter=1000)
        assert x_samps.shape == (1000,)
        assert np.abs(np.mean(x_samps) - 0) < 0.05
        assert np.abs(np.std(x_samps) - 1) < 0.05
    ...
    def test_conditioning(self):
        ...
        [x_samps] = self.sample_flat([x], [y], [1], niter=1000)
        assert x_samps.shape == (1000,)
    ...
    def test_nonrandom(self):
        ...
        x = ir.RV(ir.Normal(), loc, scale)
        y = ir.RV(ir.Add(), x, x)
        [x_samps, y_samps] = self.sample_flat([x, y], [], [], niter=100)
    ...
    def test_nonrandom_conditioning(self):
        ...
        y1 = ir.RV(ir.Add(), x, x)
        y2 = ir.RV(ir.Mul(), x, x)
        [x_samps, y1_samps, y2_samps] = self.sample_flat([x, y1, y2], [z], [1.0], niter=1000)
        assert x_samps.shape == y1_samps.shape == y2_samps.shape == (1000,)
        assert np.abs(np.mean(x_samps) - 0.5) < 0.05
        assert np.abs(np.var(x_samps) - 0.5) < 0.05
        assert np.allclose(y1_samps, x_samps * 2)
        assert np.allclose(y2_samps, x_samps**2)
