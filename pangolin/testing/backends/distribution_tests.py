...
testdata = [
    ...
    (NormalPrec, lambda a, b: scipy.stats.norm(a,1/b**2), ["real", (.1, 10.0)] ),
    ...
    (Categorical, lambda p: scipy.stats.rv_discrete(name="categorical", values=(np.arange(len(p)), p)), ["simplex"]),
        ...
        for reps in range(5):
            ...
            if expected_mean is not None:
                ...
                def testfun(samps_list):
                    ...
                    match = np.all(np.abs(empirical_mean - expected_mean) / np.abs(expected_mean) < .05)

                    if expected_std is not None:
                        empirical_std = np.std(samps, axis=0)
                        match &= np.all(np.abs(empirical_std - expected_std) / expected_std < .05)

                    if expected_cov is not None:
                        empirical_cov = np.cov(samps.T)
                        match &= np.all(np.abs(empirical_cov - expected_cov) / np.linalg.norm(expected_cov) < .05)
