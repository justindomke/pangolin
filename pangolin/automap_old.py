...
class UnmergableParentsError(Exception):
    def __init__(self,indent,trace):
        self.indent=indent
        self.trace=trace

def check_parents_compatible(x,*,indent,trace):
    N = len(x)
    M = len(x[0].parents)
    dist = x[0].cond_dist
    # print(f"{dist=}")
    ...
    for xi in x:
        ...
        if xi.cond_dist != dist:
            ...
            raise UnmergableParentsError(indent,trace)
        assert len(xi.parents) == M
...
# mergepattern is a sequence of merging instrunctions (true/false)
...
def automap(x,*,check=True):
    ...
    # while True:
    #     try:
    #         return automap_recursive(x, check=check, toplevel=True, opt=opt, verbose=False, trace=[])
    #     except UnmergableParentsError as e:
    #         print(f"COULDN'T MERGE AT {opt=}")
    #         opt = opt-1
    #         if opt < 1:
    #             raise UnmergableParentsError(-1)
    return automap_recursive(x, check=check, toplevel=True, opt=1000, verbose=False, trace=(), nomerge={})

    # nomerge = {}
    # while True:
    #     try:
    #         return automap_recursive(x, check=check, toplevel=True, opt=1000, verbose=False, trace=(), nomerge=nomerge)
    #     except UnmergableParentsError as e:
    #         print(f"COULDN'T MERGE {e.trace=}",flush=True)
    #         if trace not in nomerge:
    #             nomerge[trace] = 10
    #         else:
    #             nomerge[trace] -= 1
    #         if nomerge[trace] < 0:
    #             raise UnmergableParentsError(-1, trace)
    #
    #         #keep = len(e.trace)
    #         trace = e.trace
    #         #print(f"{trace=}")
    ...
    #         #if trace in nomerge:
    #         #    raise Exception()
    #         #while trace[:keep] in nomerge:
    #         #    #print(f"{keep=} {trace[:keep]=}",flush=True)
    #         #    keep -= 1
    #         #    if keep < 0:
    #         #        raise UnmergableParentsError(-1,())
    #
    #         #raise Exception()
    #         #nomerge[tuple(trace[:keep])] = None
    #         #nomerge[trace[:keep]] = None
    #
    #         #opt = opt-1
    #         #if opt < 1:
    #         #    raise UnmergableParentsError(-1)
...
def automap_recursive(x, *, indent=0, check, toplevel, opt, verbose, trace, nomerge):
    """
    ...
    if verbose:
        print(f'{" " * (indent * 4)}automap({opt}) called on {len(x)} inputs:',end='')
        def display(xi):
            s = ""
            if isinstance(xi, RV):
                print(
    ...
    if all(is_sequence(xi) for xi in x):
        ...
        #     trace = trace + (f'recurse',)
        #     new_xi = automap_recursive(xi, indent=indent + 1, check=check, toplevel=toplevel, opt=opt, verbose=verbose,
        #                            trace=trace + (f'recurse{i}',), nomerge=nomerge)
        #     new_x.append(new_xi)
        # trace = trace + ('merge',)
        # #new_x = [automap_recursive(xi, indent=indent + 1, check=check, toplevel=toplevel, opt=opt, verbose=verbose,
        # #                           trace=trace + [f'recurse{i}'], nomerge=nomerge) for i, xi in enumerate(x)]
        # return automap_recursive(new_x, indent=indent + 1, check=check, toplevel=toplevel, opt=opt, verbose=verbose, trace=trace, nomerge=nomerge)
    ...
    dist, N, M = check_parents_compatible(x, indent=indent, trace=trace)
    ...
    for m in range(M): # M is number of parents for each element of the sequence
        ...
        try:
            my_v, my_k = vec_args(p, indent=indent, check=check, toplevel=toplevel, opt=opt, verbose=verbose, trace=trace+ (f"vec{m}",), nomerge=nomerge)
        except UnmergableParentsError:
            my_v, my_k = vec_args(p, indent=indent, check=check, toplevel=toplevel, opt=0, verbose=verbose,
                                  trace=trace + (f"vec{m}",), nomerge=nomerge)
...
def not_allowed_to_merge(trace,nomerge):
    return any(trace[:k] in nomerge for k in range(len(trace)))

def vec_args(p,  *, indent=0, check, toplevel, opt, verbose, trace, nomerge):
    p0 = p[0]
    d0 = p0.cond_dist

    if not_allowed_to_merge(trace, nomerge):
        print(f"at {trace}, previously failed, avoiding merge")

    if all(pi is p0 for pi in p):
        # if all parents are the same, return the first one and don't map
        return (p0, None)
    elif (indent<opt) and all(deep_equal(pi,p0) for pi in p):
        print("dangerous equality")
        if verbose: print(f'{" " * (indent * 4)}DEEP EQUALITY FOUND')
    ...
    try:
        ...
        p_vec = automap_recursive(p, indent=indent + 1, check=check, toplevel=False, opt=opt, verbose=verbose,trace=trace+(f'vec',),nomerge=nomerge)
        return p_vec, 0
